...
productos.forEach(producto => {
    const html = `
            <article class="producto-item" data-id="${producto.id}">
                <div><img src="${producto.image}" alt="Producto"></div>
                <h3>${producto.title}</h3>
                <p>Descripción producto ${producto.id}</p>
...
document.addEventListener("click", (event)=>{
    ...
    if(event.target.classList.contains("agregar")){
        ...
        // usando destructuring preparao articulo para ser agregado
        const {category, description, image, price, title} = elemento;
        const producto = {
            id,
            title,
            price,
